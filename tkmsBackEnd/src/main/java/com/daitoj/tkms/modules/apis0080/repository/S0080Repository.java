  ...
  @Query(
      """
            SELECT new com.daitoj.tkms.modules.apis0080.service.dto.PartnerVendorApprInfoDto(
                  mv.partnerVendorCd,
                  CONCAT(mv.compNm, mv.branchNm),
                  CASE WHEN mv.demolPermFlg = '1'    THEN 'あり' ELSE 'なし' END,
                  CASE WHEN mv.securityCertFlg = '1' THEN 'あり' ELSE 'なし' END,
                  CASE WHEN vi.id IS NOT NULL        THEN 'あり' ELSE 'なし' END,
                  me.empNm,
        ...
        INNER JOIN MItemListSetting il    ON wa.apprSt = il.id.itemCd
             ...
             WHERE wr.businessTypeCd.businessTypeCd = 'B00008'
               AND wr.businessTblId                 = 'M_VENDOR'
               AND wa.apprSt                        = '1'
