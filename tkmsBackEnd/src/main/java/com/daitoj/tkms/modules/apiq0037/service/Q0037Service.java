package com.daitoj.tkms.modules.apiq0037.service;
...
import com.daitoj.tkms.modules.common.repository.VsubbieVendorRelRepository;
...
public class Q0037Service {
  private static final Logger LOG = LoggerFactory.getLogger(Q0037Service.class);
  ...
  /** 下請業者関係ビュー. */
  private final VsubbieVendorRelRepository vsubbieVendorRelRepository;
  ...
  public Q0037Service(
      ...
      VsubbieVendorRelRepository vsubbieVendorRelRepository,
    ...
    this.vsubbieVendorRelRepository = vsubbieVendorRelRepository;
      ...
      for (int i = 0; i < printData.getConstrWbsDtls().size(); i++) {
        if (i % reportMaxRowCount == 0) {
          List<TConstrWbsDtl> onePageRows =
              printData
                  .getConstrWbsDtls()
                  .subList(i, Math.min(i + reportMaxRowCount, printData.getConstrWbsDtls().size()));
